...
def _tool_call_response(query="test"):
    """Mock _stream_model return for a web_search tool call: (content, message)."""
    tool_call = MagicMock()
...
def _final_response(content="Answer."):
    """Mock _stream_model return for a final answer: (content, message)."""
    message = MagicMock()
...
def test_historical_fact_no_search(orchestrator):
    ...
    with patch.object(orchestrator, '_stream_model', return_value=_final_response("William Shakespeare wrote Hamlet.")), \
...
def test_current_event_triggers_search(orchestrator):
    ...
    with patch.object(orchestrator, '_stream_model', side_effect=[
...
def test_general_knowledge_no_search(orchestrator):
    ...
    with patch.object(orchestrator, '_stream_model', return_value=_final_response("Photosynthesis is the process...")), \
         patch.object(orchestrator.search_engine, 'search') as mock_search:
