...
import core.github_tools as github_tools
...
def mock_token():
    ...
    with patch("core.github_tools.subprocess.run") as m:
        proc = MagicMock()
...
def test_list_repos_returns_names():
    ...
    with patch("core.github_tools._gh", return_value=_resp(200, repos)):
...
def test_list_repos_api_error():
    with patch("core.github_tools._gh", return_value=_resp(401, {"message": "Bad credentials"})):
        result = github_tools.github_list_repos()
...
def test_read_file_decodes_base64():
    ...
    with patch("core.github_tools._gh", return_value=_resp(200, payload)):
...
def test_read_file_not_found():
    with patch("core.github_tools._gh", return_value=_resp(404, {"message": "Not Found"})):
...
def test_list_files_directory():
    ...
    with patch("core.github_tools._gh", return_value=_resp(200, entries)):
...
def test_list_files_not_found():
    with patch("core.github_tools._gh", return_value=_resp(404, {})):
...
def test_write_file_creates_new():
    ...
    with patch("core.github_tools._gh", side_effect=[existing_404, commit_resp]):
...
def test_write_file_updates_existing():
    ...
    with patch("core.github_tools._gh", side_effect=[existing_200, commit_resp]):
...
def test_write_file_api_error():
    ...
    with patch("core.github_tools._gh", side_effect=[existing_404, _resp(422, {"message": "Validation Failed"})]):
...
def test_create_repo_success():
    ...
    with patch("core.github_tools._gh", return_value=_resp(201, payload)):
...
def test_create_repo_duplicate_name():
    with patch("core.github_tools._gh", return_value=_resp(422, {"message": "name already exists"})):
...
def test_create_branch_success():
    ...
    with patch("core.github_tools._gh", side_effect=[repo_resp, ref_resp, create_resp]):
...
def test_create_branch_already_exists():
    repo_resp = _resp(200, {"default_branch": "main"})
    ref_resp  = _resp(200, {"object": {"sha": "basesha"}})
    with patch("core.github_tools._gh", side_effect=[repo_resp, ref_resp, _resp(422, {})]):
...
def test_list_issues_excludes_prs():
    ...
    with patch("core.github_tools._gh", return_value=_resp(200, items)):
...
def test_list_prs():
    ...
    with patch("core.github_tools._gh", return_value=_resp(200, prs)):
...
def test_search_code():
    ...
    with patch("core.github_tools._gh", return_value=_resp(200, body)):
...
def test_delete_file_with_confirm_deletes():
    ...
    with patch("core.github_tools._gh", side_effect=[existing, delete]):
...
def test_delete_branch_with_confirm_deletes():
    with patch("core.github_tools._gh", return_value=_resp(204, {})):
...
def test_delete_branch_api_error():
    with patch("core.github_tools._gh", return_value=_resp(422, {"message": "branch not found"})):
